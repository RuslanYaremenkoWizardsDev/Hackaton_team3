        ...
        public string[] GetParticipantFromDbByMatch(string matchId)
        {
            if (TableHasValues("Matches") && TableHasValues("Participants"))
            {
                if (matchId != null)
                {
                    ...
                    {
                        string query =
                            $"select * from [dbo].[Participants] where id = (select [ParticipantOne] from [dbo].[Matches] where [dbo].[Matches].[ID]={matchId}) ";
                        SqlCommand command = new SqlCommand(query, _sqlConnectionTournamentsDb);
                        SqlDataReader reader = command.ExecuteReader();
                    ...
                    {
                        string query =
                            $"select * from [dbo].[Participants] where id = (select [ParticipantTwo] from [dbo].[Matches] where [dbo].[Matches].[ID]={matchId})";
                        SqlCommand command = new SqlCommand(query, _sqlConnectionTournamentsDb);
                        SqlDataReader reader = command.ExecuteReader();
                        if (reader.Read())
                        {
